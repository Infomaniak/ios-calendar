...
struct TimelineBackgroundView: View {
    ...
    enum Constants {
        ...
        static let verticalInset = labelFontSize / 2
        ...
        static let leadingInset: CGFloat = {
            ...
            let labelWidth = fakeDate.formatted(dateFormater)
                .size(withAttributes: [.font: font]).width + 2 * IKPadding.mini

            return labelWidth.rounded(.up) + labelSpacing
        }()
    }
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for (index, mark) in hourMarks.enumerated() {
                ...
                let yPosition = CGFloat(index) * pointsPerHour + verticalOffset
                var path = Path()
                ...
                context.draw(
                    ...
                    in: CGRect(
                        ...
                        y: yPosition - Constants.labelFontSize / 2,
